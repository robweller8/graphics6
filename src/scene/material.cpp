  ...
  // like this:
   		...
   		++litr )
   {
   		Light* pLight = *litr;
                ...
                Vec3d normal = i.N;
                normal.normalize();
                Camera view = scene->getCamera();
                Vec3d eye = view.getEye();
                Vec3d viewing = eye - position;
                viewing.normalize();
                double intensity = (*pLight).distanceAttenuation(position);
                Vec3d reflect = normal*(2*(normal*lightDirection)) - lightDirection;
                reflect.normalize();
